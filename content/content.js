...
(function () {
  ...
  function extractAllColors(options = {}) {
    ...
    const elementsToProcess =
      maxElements > 0 ? Array.from(allElements).slice(0, maxElements) : Array.from(allElements);
  ...
  function normalizeFontFamily(fontFamily) {
    ...
    const families = cleaned.split(',').map(f => f.trim().replace(/^["']|["']$/g, ''));
  ...
  function parseFontSize(fontSize) {
    ...
    // Accept any valid font-size value (px, em, rem, %, etc.)
    return trimmed;
  }

  function normalizeFontWeight(fontWeight) {
    if (!fontWeight) return '400';
    const weight = String(fontWeight).trim();
  ...
  function normalizeFontStyle(fontStyle) {
    ...
    return ['normal', 'italic', 'oblique'].includes(style) ? style : 'normal';
  }

  function createFontSignature(fontData) {
    return `${fontData.family}|${fontData.size}|${fontData.weight}|${fontData.style}`;
  }

  // ========== Font Extraction Functions ==========

  function extractFontsFromElement(element, computedStyle) {
    const fonts = [];
    ...
    const fontWeight = normalizeFontWeight(computedStyle.fontWeight);
    const fontStyle = normalizeFontStyle(computedStyle.fontStyle);
    ...
    // Extract font if we have a font family
    // fontSize should always be available from computed styles, but use default if missing
    if (fontFamily) {
      // Use computed fontSize, or default to '16px' if somehow missing
      const effectiveSize = fontSize || '16px';
      ...
      fonts.push({
        ...
        source: 'element',
        element: tagName,
        originalFamily: computedStyle.fontFamily,
      });
    }
  ...
  function extractFontsFromStylesheets() {
    const fonts = [];
    const styleSheets = Array.from(document.styleSheets);
        ...
        rules.forEach(rule => {
          if (rule.style) {
            ...
            const fontSize = parseFontSize(
              rule.style.fontSize || rule.style.getPropertyValue('font-size')
            );
            const fontWeight = normalizeFontWeight(
              rule.style.fontWeight || rule.style.getPropertyValue('font-weight')
            );
            const fontStyle = normalizeFontStyle(
              rule.style.fontStyle || rule.style.getPropertyValue('font-style')
            );
            const lineHeight =
              rule.style.lineHeight || rule.style.getPropertyValue('line-height') || 'normal';
            const letterSpacing =
              rule.style.letterSpacing || rule.style.getPropertyValue('letter-spacing') || 'normal';
            const textTransform =
              rule.style.textTransform || rule.style.getPropertyValue('text-transform') || 'none';
            const textDecoration =
              rule.style.textDecoration || rule.style.getPropertyValue('text-decoration') || 'none';

            // Extract font from stylesheet if font-family is present
            // fontSize is optional in stylesheets (can be inherited)
            if (fontFamily) {
              // Use fontSize if available, otherwise use a default
              const effectiveSize = fontSize || '16px';
  ...
  function extractAllFonts(options = {}) {
    const { maxElements = 0, includeStylesheets = true } = options;
    const fontMap = new Map();

    // Get all text-containing elements (more efficient than all elements)
    const allElements = document.querySelectorAll('*');
    const elementsToProcess =
      maxElements > 0 ? Array.from(allElements).slice(0, maxElements) : Array.from(allElements);
    ...
    elementsToProcess.forEach(element => {
      try {
        // Process all visible elements (computed styles will show the actual font being used)
        // The extractFontsFromElement function will filter out hidden/non-text elements
        const computedStyle = window.getComputedStyle(element);
        const elementFonts = extractFontsFromElement(element, computedStyle);

        elementFonts.forEach(fontObj => {
          const signature = createFontSignature(fontObj);

          if (!fontMap.has(signature)) {
            fontMap.set(signature, {
              family: fontObj.family,
              size: fontObj.size,
              weight: fontObj.weight,
              style: fontObj.style,
              lineHeight: fontObj.lineHeight,
              letterSpacing: fontObj.letterSpacing,
              textTransform: fontObj.textTransform,
              textDecoration: fontObj.textDecoration,
              frequency: 0,
              sources: [],
              selectors: [],
              elements: [],
          ...
          const fontData = fontMap.get(signature);
          fontData.frequency++;

          if (fontObj.source === 'element' && !fontData.elements.includes(fontObj.element)) {
            fontData.elements.push(fontObj.element);
          }

          if (
            fontObj.source === 'stylesheet' &&
            fontObj.selector &&
            !fontData.selectors.includes(fontObj.selector)
          ) {
            fontData.selectors.push(fontObj.selector);
          }

          if (!fontData.sources.includes(fontObj.source)) {
            fontData.sources.push(fontObj.source);
          }
        });
      } catch (e) {
        // Skip elements that can't be processed
    ...
    if (includeStylesheets) {
      try {
        const stylesheetFonts = extractFontsFromStylesheets();
        stylesheetFonts.forEach(fontObj => {
          const signature = createFontSignature(fontObj);
          ...
          if (fontObj.selector && !fontData.selectors.includes(fontObj.selector)) {
            fontData.selectors.push(fontObj.selector);
          }
    ...
    const fontsArray = Array.from(fontMap.values());
    fontsArray.sort((a, b) => b.frequency - a.frequency);
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'extractColors') {
      try {
        ...
        const maxElements =
          elementCount > CONFIG.LARGE_PAGE_THRESHOLD ? CONFIG.MAX_SAMPLED_ELEMENTS : 0;
    ...
    if (request.action === 'extractFonts') {
      try {
        ...
        sendResponse({
          success: false,
          error: error.message || 'Failed to extract fonts',
        });
      }
